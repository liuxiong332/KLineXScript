use super::{ConvertErr, DataType, PineFrom, PineStaticType, PineType, SecondType, NA};
...
pub trait SeriesCall<'a> {
    ...
    fn run(&self, _context: &mut dyn Ctx<'a>) -> Result<(), ConvertErr> {
        Ok(())
    }

    fn back(&self, _context: &mut dyn Ctx<'a>) -> Result<(), ConvertErr> {
        Ok(())
    }
...
impl<'a> Callable<'a> {
    ...
    pub fn back(&self, context: &mut dyn Ctx<'a>) -> Result<(), ConvertErr> {
    ...
    pub fn run(&self, context: &mut dyn Ctx<'a>) -> Result<(), ConvertErr> {
        if let Some(ref caller) = self.caller {
            caller.run(context)
        } else {
            Ok(())
